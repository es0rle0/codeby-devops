...
# Используем уже существующую VPC (созданную config1)
data "yandex_vpc_network" "vpc" {
  name = var.existing_vpc_name
}

# Route table для private subnet: дефолтный маршрут через уже существующий NAT gateway (из config1)
resource "yandex_vpc_route_table" "rt_private" {
  ...
  static_route {
    ...
    gateway_id         = var.existing_nat_gateway_id
  }
}
...
resource "yandex_compute_instance" "vm_private" {
  ...
  # Установка Nginx + перевод на 8080
  provisioner "remote-exec" {
    inline = [
      "sudo apt-get update -y",
      "sudo apt-get install -y nginx",
  ...
  # Подключаемся к private VM через bastion (public VM)
  connection {
    type        = "ssh"
